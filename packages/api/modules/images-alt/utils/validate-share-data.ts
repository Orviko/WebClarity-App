...
const MAX_ARRAY_LENGTH = 10000; // Maximum array length (max 10,000 images)
...
function validateImageUrl(url: string): void {
	...
	// Basic URL validation - should start with http:// or https:// or be a relative URL
	if (
		!url.startsWith("http://") &&
...
/**
 * Validates the incoming Images Alt share data
 */
export function validateShareData(data: ShareDataInput): {
	...
	const imagesDataObj = data.imagesData as Record<string, unknown>;

	// Validate required sections exist
	if (!imagesDataObj.summary || !imagesDataObj.images) {
		throw new ORPCError("BAD_REQUEST");
	}

	// Validate images array
	if (!Array.isArray(imagesDataObj.images)) {
		throw new ORPCError("BAD_REQUEST");
	}

	// Validate each image URL
	for (const image of imagesDataObj.images) {
		if (typeof image === "object" && image !== null) {
			const imageObj = image as Record<string, unknown>;
			if (typeof imageObj.url === "string") {
				validateImageUrl(imageObj.url);
			}
		}
	}
	...
	try {
		validateJsonDepth(data.imagesData);
