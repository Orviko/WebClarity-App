...
/**
 * Validates the incoming share data structure and size
 */
export function validateShareData(data: ShareDataInput): {
	...
	// Allow: example.com, subdomain.example.com, localhost, 127.0.0.1, etc.
	const domainRegex =
	...
	// At least one of typographyData or colorsData must be present
	if (!data.typographyData && !data.colorsData) {
		throw new ORPCError("BAD_REQUEST");
	}

	// Validate JSON structure depth and prevent deeply nested/abusive structures
	try {
		if (data.typographyData) {
			validateJsonDepth(data.typographyData);
		}
		if (data.colorsData) {
			validateJsonDepth(data.colorsData);
		}
	...
	return {
		typographyData: data.typographyData ?? null,
