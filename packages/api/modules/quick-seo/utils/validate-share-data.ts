...
/**
 * Validates the incoming Quick SEO share data
 */
export function validateShareData(data: ShareDataInput): {
	...
	// Validate seoData structure
	if (!data.seoData || typeof data.seoData !== "object") {
		throw new ORPCError("BAD_REQUEST");
	}

	const seoDataObj = data.seoData as Record<string, unknown>;

	// Validate required sections exist
	if (
		!seoDataObj.openGraph ||
		!seoDataObj.meta ||
		!seoDataObj.content ||
		!seoDataObj.technical
	) {
		throw new ORPCError("BAD_REQUEST");
	}
	...
	try {
		validateJsonDepth(data.seoData);
