...
/**
 * Shared handler logic for creating a Quick SEO share
 */
export async function createShareHandler(
	...
	const result = await db.$transaction(async (tx) => {
		// Create Quick SEO data
		// Cast to any to satisfy Prisma's Json type requirements
		const quickSeoData = await tx.quickSEOData.create({
			data: {
				seoData: validated.seoData as any,
		...
		const share = await tx.share.create({
			data: {
				shareId,
				type: ShareType.QUICK_SEO,
				expiresAt,
				title,
				websiteUrl: validated.websiteUrl,
				userId,
				organizationId,
				quickSeoDataId: quickSeoData.id,
			},
		});
		...
		// TODO: Generate share OG image (optional for now)
		// Can be added later similar to heading structure
...
export const createShare = publicProcedure
	.route({
		...
		path: "/create-share",
		tags: ["Quick SEO"],
		summary: "Create a public share for Quick SEO data",
		description:
			"Creates a share for Quick SEO analysis data. Authenticated users get 7 days, anonymous users get 24 hours.",
	})
