...
export const getShare = publicProcedure
	.route({
		...
		path: "/get-share",
		tags: ["Quick SEO"],
		summary: "Get Quick SEO share data",
		description: "Retrieves Quick SEO analysis data by share ID",
	})
	.input(
		z.object({
			shareId: z
	...
	.handler(async ({ input }) => {
		...
		// Verify share type
		if (share.type !== ShareType.QUICK_SEO) {
			throw new ORPCError("NOT_FOUND");
		}
		...
		return {
			seoData: share.quickSeoData.seoData,
			exportOptions: share.quickSeoData.exportOptions,
