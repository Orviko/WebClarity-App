...
/**
 * Shared handler logic for creating a Social View share
 */
export async function createShareHandler(
	...
	// Default title format: "Social Media Preview - {websiteUrl}"
	const title = `Social Media Preview - ${validated.websiteUrl}`;

	// Create Social View data and share in a transaction
	const result = await db.$transaction(async (tx) => {
		// Create Social View data
		const socialViewData = await tx.socialViewData.create({
			data: {
				socialData: validated.socialData as any,
		...
		const share = await tx.share.create({
			data: {
				shareId,
				type: ShareType.SOCIAL_VIEW,
				expiresAt,
				title,
				websiteUrl: validated.websiteUrl,
				userId,
				organizationId,
				socialViewDataId: socialViewData.id,
			},
		});
...
export const createShare = publicProcedure
	.route({
		...
		path: "/create-share",
		tags: ["Social View"],
		summary: "Create a public share for Social View data",
		description:
			"Creates a share for Social View analysis data. Authenticated users get 7 days, anonymous users get 24 hours.",
	})
