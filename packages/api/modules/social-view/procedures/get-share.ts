...
export const getShare = publicProcedure
	.route({
		...
		path: "/get-share",
		tags: ["Social View"],
		summary: "Get Social View share data",
		description: "Retrieves Social View analysis data by share ID",
	})
	.input(
		z.object({
			shareId: z
	...
	.handler(async ({ input }) => {
		...
		// Verify share type
		if (share.type !== ShareType.SOCIAL_VIEW) {
			throw new ORPCError("NOT_FOUND");
		}
		...
		return {
			socialData: share.socialViewData.socialData,
			exportOptions: share.socialViewData.exportOptions,
